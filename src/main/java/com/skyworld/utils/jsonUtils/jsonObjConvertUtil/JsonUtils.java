package com.skyworld.utils.jsonUtils.jsonObjConvertUtil;
...
public class JsonUtils {
	...
	public static void main(String[] args) {
		/// 读取json字符串
		if(args.length == 0){
			throw new RuntimeException("请输入json路径");
		}
		String json = FileUtils.readToString(new File(args[0]), "UTF-8");
		parseJson2Java(json);
	}
	...
	public static void parseJson2Java(String jsonStr) {
		...
		List<Json2JavaElement> jsonBeanTree = getJsonBeanTree(jsonStr);
 		...
 		FileUtils.writeString2File(javaBeanStr, new File("c:"+File.separator+"JsonBean.java"));
	}
